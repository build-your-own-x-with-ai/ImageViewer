...
> 状态：阶段 2（`v0.2.0`）。每阶段结束时追加，阶段 5 定稿。

## 1. 代码规模

`lib/` 下 45 个文件、9000 行（含注释与文档注释，本项目注释占比很高，
是刻意的）。括号里是阶段 1 的数字。

| 层 | 行数 | 说明 |
|---|---|---|
| `core/` | 1030 | 字节/位读取器、`RgbaImage`、注册表、异常 |
| `compress/` | 1069（新） | inflate 一家：`inflate.dart` 647 行 + Huffman 218 + 码表 130 + Adler 74 |
| `codecs/bmp/` | 1069 | 六个头部版本是大头（`bmp_header.dart` 429 行） |
| `codecs/png/` | 1766（新） | 九个文件，最大的 `png_decoder.dart` 也只 482 行 |
| `codecs/pnm/` | 634 | 词法处理占一半 —— 文本格式的空白与注释规则很啰嗦 |
| `codecs/yuv/` | 766 | 九种布局压成三个循环，靠 `YuvFormat` 描述子 |
| `platform/` | 438 | 条件导入 + `dart:io` 隔离 |
| `services/` | 101 | isolate 调度与计时 |
| `ui/` | 2070 | 外壳。`viewer_page.dart` 506 行是状态机 |
| `main.dart` | 57 | 只装主题、指首页 |

解码器（`core` + `compress` + `codecs`）共 6334 行（阶段 1 是 3499），
UI 2070 行没变。**解码逻辑比界面多**，而且差距在拉大 —— 对一个「自己写
解码器」的项目来说这个比例是健康的。

阶段 2 加了 2837 行，其中 1069 行是 `compress/`。**一个格式的成本里，
有 38% 花在了不属于这个格式的地方** —— 这个数字是第 4 节的由来。

测试 496 个（阶段 1 是 305）：core 68、compress 67、BMP 76、PNG 97、
...
时**不需要改任何 UI 代码**。这条阶段 1 立下的设计假设在阶段 2 得到了验证：
PNG 的元数据比前三种格式丰富得多（多了 chunk 数、deflate 块类型、滤波器
用量、解压后大小、`gAMA`/`pHYs`/`tEXt`），**UI 代码一行没改**。

唯一为 PNG 改动的 UI 代码是文件选择器的扩展名白名单加了一行 `'png'` ——
那不是渲染逻辑，是「能选什么文件」的策略。
...
- 文件选择器的扩展名白名单只列已实现的格式（阶段 2 加了 `png`，现在还差
  jpeg/webp）。**不能**照抄侧栏的 `kBrowsableExtensions` —— 那份含所有六种
  格式，用户若选中一张正常的 JPEG 会得到「没有解码器认领」+「试试按裸 YUV
  解码」，而这个建议是错的。每加一个解码器要同时看这两处，方向相反：
  选择器收紧，侧栏放宽。
